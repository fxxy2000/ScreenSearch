...
public class AssistVisualizer extends View {
    ...
    public void logTree() {
        if (mAssistStructure != null) {
            if (mAssistStructure.getWindowNodeCount() >0 ) {
                AssistStructure.WindowNode windowNode = mAssistStructure.getWindowNodeAt(0);
                dumpViewNode(windowNode.getRootViewNode());
            }
        }
    }

    private void dumpViewNode(AssistStructure.ViewNode viewNode) {
        for (int i=0;i<viewNode.getChildCount();i++) {
            AssistStructure.ViewNode childNode = viewNode.getChildAt(i);
            Log.i(TAG, "->"+childNode.getClassName());
            Log.i(TAG, "-->Package:"+childNode.getId());
            Log.i(TAG, "-->Package:"+childNode.getIdPackage());
            if (!TextUtils.isEmpty(childNode.getText())) {
                Log.i(TAG, "-->Text:"+childNode.getText());
            }

            if (!TextUtils.isEmpty(childNode.getContentDescription())) {
                Log.i(TAG, "-->ContentDesc:"+childNode.getContentDescription());
            }

            if (childNode.getChildCount() > 0) {
                dumpViewNode(childNode);
            }
        }
    }
